...
#define MUSIC_START	_music_data
...
;
; Increment by one every 50th of a second
;
_TimeIrqHandler
	pha
...
_VSync
		sei
	lda _CurrentSync
	sta _CurrentSyncCopy
	lda _CurrentSync+1
	sta _CurrentSyncCopy+1
		cli

	;
	; Increment current VBL value		 
	;
	inc _LastSync
	bne sync_end
	inc _LastSync+1
...
sync_loop
	...
	lda _CurrentSyncCopy+1
	cmp _LastSync+1	
	bcc sync_loop

	lda _CurrentSyncCopy
	cmp _LastSync
	bcc sync_loop
